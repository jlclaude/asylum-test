...
import {
  adjacentWheelId,
  defaultGameModeActiveWheelId,
  nextUnfinishedWheelId,
  unfinishedWheelIds,
} from "../lib/game-mode-operator";
...
import {
  emitDesktopAutomationEvent,
  updateDesktopActiveGame,
} from "../lib/desktop-automation.client";
...
export default function GameModePage() {
  ...
  const beginFetcher = useFetcher<WheelActionData>();
  ...
  useEffect(() => {
    ...
    updateDesktopActiveGame({
      ...
      publicClaimUrl: `${origin}/games/${game.id}`,
      facebookPost: `${game.title} · ${game.raffleCode}`,
    });
  }, [csrfToken, game.id, game.raffleCode, game.title, routeMode]);
  ...
  const [operatorStates, setOperatorStates] = useState<
    Record<string, WheelOperatorState>
  >({});
  ...
  const [acceptedIds, setAcceptedIds] = useState<Set<string>>(
    () =>
      new Set(
        orderedWheels
          .filter((wheel) => wheel.resultAcceptedAt)
          .map((wheel) => wheel.id),
      ),
  );
  ...
  useEffect(() => {
    ...
    if (
      activeWheelId &&
      orderedWheels.some((wheel) => wheel.id === activeWheelId)
    )
      return;
  ...
  const updateOperatorState = useCallback((state: WheelOperatorState) => {
    setOperatorStates((current) => {
      ...
      if (
        ...
        previous.selectedDuration === state.selectedDuration &&
        previous.spinning === state.spinning
      )
        return current;
  ...
  useEffect(() => {
    ...
    if (response.stale) {
      if (handledStaleAcceptance.current !== response) {
        ...
        setOperatorMessage(
          response.error ??
            "This wheel changed in another session. The latest state has been loaded.",
        );
      }
      return;
    }
    if (
      response.intent !== "accept-result" ||
      !response.success ||
      !response.wheelId
    )
      return;
    ...
    const nextId = nextUnfinishedWheelId(
      orderedWheels.map((wheel) =>
        completedLocally.has(wheel.id)
    ...
    setOperatorMessage(
      `${orderedWheels.find((wheel) => wheel.id === nextId)?.label ?? "Next wheel"} selected.`,
    );
  ...
  const activeWheel = activeWheelId
    ? (operatorStates[activeWheelId] ??
      (() => {
        const wheel = orderedWheels.find(
          (candidate) => candidate.id === activeWheelId,
        );
        return wheel
          ? {
              id: wheel.id,
              label: wheel.label,
              status: wheel.status,
              selectedDuration: wheel.spinDurationSeconds,
              spinning: wheel.status === "SPINNING",
            }
          : null;
      })())
    : null;
  ...
  const runActiveAction = useCallback(
    (action: WheelOperatorAction): WheelOperatorResult => {
      if (!activeWheelId)
        return {
          triggered: false,
          message: "No unfinished wheel is selected.",
        };
      return (
        wheelRefs.current.get(activeWheelId)?.runAction(action) ?? {
          triggered: false,
          message: "Active wheel controls are unavailable.",
        }
      );
    },
    [activeWheelId],
  ...
  const navigateWheel = useCallback(
    (direction: 1 | -1): WheelOperatorResult => {
      if (activeWheel?.spinning)
        return { triggered: false, message: "Wheel is currently spinning." };
      if (unfinishedWheels.length === 0)
        return { triggered: false, message: "All wheels are completed." };
      const nextId = adjacentWheelId(
        unfinishedWheels.map((wheel) => wheel.id),
        activeWheelId,
        direction,
      );
      ...
      if (!nextWheel)
        return {
          triggered: false,
          message: "No unfinished wheel is available.",
        };
      setFocusReleased(false);
    ...
    () => ({
      ...
      toggleFullscreen: () => {
        void toggleFullscreen().then((success) => {
          if (!success)
            setOperatorMessage("Fullscreen is unavailable in this browser.");
        });
      ...
      toggleSound: () => {
        ...
        return {
          triggered: true,
          message: muted ? "Wheel effects enabled." : "Wheel effects muted.",
        };
      },
  ...
  const firstNameWheel = run?.rounds[0]?.wheels.find(
    (wheel) => wheel.type === "NAME",
  );
  ...
  return (
    <main ref={fullscreenTarget} className="studio-page" style={variables}>
      <div className="studio-shell">
        <header className="studio-topbar">
          ...
          <div className="studio-topbar-center">
            ...
            <strong>CONTAINMENT CONTROL SYSTEM</strong>
            <span aria-hidden="true" />
          </div>
        ...
        <div className="studio-toolbar">
          ...
          <select
            value={themeKey}
            onChange={(event) =>
              setThemeKey(event.target.value as AsylumThemeKey)
            }
            aria-label="Asylum theme"
          >
            {Object.values(ASYLUM_THEMES).map((option) => (
        ...
        {game.archivedAt ? (
          <div className="studio-begin-error">
            This game is archived. Results remain readable, but gameplay
            controls are disabled until it is restored.
          </div>
        ) : null}
        ...
        <GameIdentityCard
          ...
          status={game.status}
          nameWheelCount={game.wheelCount}
          totalEntries={firstNameWheel?.entries.length}
          themeLabel={theme.label}
        />

        {!run ? (
          <section className="studio-begin-panel">
            <p>Authorized initialization</p>
            <h1>Build the containment wheels</h1>

            <span>
              Confirmed paid claims will be frozen into {game.wheelCount}{" "}
              containment wheels plus one weighted Reward Chamber.
            </span>
            ...
            <beginFetcher.Form method="post">
              ...
              <input type="hidden" name="intent" value="begin-game" />

              <button
                type="submit"
                disabled={
                  beginFetcher.state !== "idle" ||
                  game.status !== "CLOSED" ||
          ...
          <>
            {run.rounds.map((round) => (
              <section className="studio-round" key={round.id}>
                <header className="studio-round-header">
                  <div>
                    <p>Active containment round</p>
                    <h1>{round.title}</h1>
                  </div>
                ...
                <div className="studio-wheel-stack">
                  {round.wheels.map((wheel, index) => (
                    <WheelSection
                      ...
                      secondChanceResult={secondChanceResultForWheel(
                        secondChance,
                        wheel,
                      )}
        ...
        <footer className="studio-statusbar">
          ...
          <strong>STANDBY FOR CONTAINMENT ENGAGEMENT</strong>

          <span>S = SHUFFLE · T = TIME · SPACE = SPIN</span>
        </footer>
      </div>
    </main>
  );
...
export function ErrorBoundary() {
  ...
  let message = "Game Mode could not be loaded.";
