...
export function GameCompletionCard({
  ...
  const names = wheels.filter(
    (wheel) => wheel.type === "NAME" && wheel.status === "COMPLETED",
  );
  const value = wheels.find(
    (wheel) => wheel.type === "VALUE" && wheel.status === "COMPLETED",
  );

  return (
    <section
      className="game-completion-card"
      aria-labelledby="containment-complete-heading"
    >
      ...
      <ul>
        {names.map((wheel, index) => (
          <li key={`${wheel.label}-${index}`}>
            <span>{wheel.label}</span>
            <strong>{wheel.winner}</strong>
          </li>
        ))}
        {value ? (
          <li>
            <span>{value.label}</span>
            <strong>{value.winner}</strong>
          </li>
        ) : null}
      </ul>
      {secondChance ? (
        <section
          className="game-completion-second-chance"
          aria-labelledby="completion-second-chance-heading"
        >
          <p>Two persisted drawing entries</p>
          <h3 id="completion-second-chance-heading">
            SECOND CHANCE FREE ENTRIES
          </h3>
          <dl>
            <div>
              <dt>Offset used</dt>
              <dd>{formatOrdinal(secondChance.offset)} eligible entry</dd>
            </div>
          </dl>
          <div className="game-completion-second-chance-grid">
            <article>
              ...
              <strong>
                {secondChance.beforeDisplayName ?? "No eligible winner"}
              </strong>
              <span>
                {secondChance.beforeDisplayName
                  ? "FREE ENTRY AWARDED"
                  : "NOT AWARDED"}
              </span>
            </article>
            <article>
              ...
              <strong>
                {secondChance.afterDisplayName ?? "No eligible winner"}
              </strong>
              <span>
                {secondChance.afterDisplayName
                  ? "FREE ENTRY AWARDED"
                  : "NOT AWARDED"}
              </span>
            </article>
