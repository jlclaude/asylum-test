...
export const WheelSection = forwardRef<WheelOperatorHandle, WheelSectionProps>(
  function WheelSection(
    ...
    const completedRestRotation =
      wheel.status === "COMPLETED" && wheel.winnerEntryIndex !== null
    ...
    const [spinning, setSpinning] = useState(wheel.status === "SPINNING");
    ...
    const selectedDuration =
      fetcher.data?.wheelId === wheel.id && fetcher.data.spinDurationSeconds
        ? fetcher.data.spinDurationSeconds
        : wheel.spinDurationSeconds;

    const busy =
      fetcher.state !== "idle" || spinning || countdownLabel !== null;
    ...
    const submitReadyAction = useCallback(
    ...
    const submitSpin = useCallback(() => {
      ...
      if (!broadcastCountdown) {
        ...
        return {
          triggered: true,
          message: `${wheel.label}: command submitted.`,
        };
      }
      ...
      if (!shouldAnimateBroadcastCountdown(reducedMotion)) {
        ...
        countdownTimers.current.push(
          window.setTimeout(() => setCountdownLabel(null), 700),
        );
        return {
          triggered: true,
          message: `${wheel.label}: containment engaged.`,
        };
      }
    ...
    useImperativeHandle(
      ...
      () => ({
        runAction: (action: WheelOperatorAction) => {
          ...
          const blocked = wheelActionBlockReason(action, {
            status: wheel.status,
            spinning,
    ...
    const uniqueCount = useMemo(() => {
      if (wheel.type === "VALUE") {
        return new Set(wheel.entries.map(entryLabel)).size;
      }
    ...
    useEffect(() => {
      if (wheel.status === "COMPLETED" && !revealActive.current) {
        ...
        stopMusic(wheel.id);
        setResult(wheel.winnerDisplayName ?? wheel.winnerValue);
        setSpinning(false);
      }
    }, [
      stopMusic,
    ...
    useEffect(() => {
      ...
      void startMusic(wheel.id, wheel.spinDurationSeconds - remainingSeconds);
    ...
    return (
      <section
        ...
        className={[
          ...
          wheel.type === "VALUE" ? "studio-wheel-section-value" : "",
        ].join(" ")}
      ...
      >
        <div className="studio-wheel-heading">
          <div>
            ...
            <h2>
              <span>{wheel.type === "VALUE" ? "$" : sequenceNumber}</span>
              {wheel.label}
            </h2>
          </div>
        ...
        <div className="studio-wheel-layout">
          <div className="studio-wheel-display">
            <WheelCanvas
              ...
              winnerEntryIndex={
                wheel.winnerEntryIndex ?? fetcher.data?.winnerEntryIndex ?? null
              }
            ...
            {countdownLabel ? (
              <div
                className="studio-broadcast-countdown"
                role="status"
                aria-live="assertive"
              >
                ...
                onReveal={() => {
                  ...
                  revealDismissTimer.current = window.setTimeout(() => {
                    if (import.meta.env.DEV)
                      console.debug("[Asylum wheel] celebration ended", {
          ...
          <WheelConsole label={wheel.label} status={visibleStatus}>
            <div className="studio-console-metrics">
              ...
              <div>
                <span>
                  {wheel.type === "NAME" ? "Unique players" : "Unique values"}
                </span>
            ...
            <div className="studio-console-controls">
              ...
              >
                ...
                <button
                  ...
                  type="submit"
                  disabled={!ready || busy || !selectedDuration}
                >
                  <span aria-hidden="true">◉</span>
                  <strong>{spinning ? "SPINNING" : "SPIN WHEEL"}</strong>
            ...
            result &&
            (wheel.status === "COMPLETED" ||
              (fetcher.data?.intent === "complete-wheel" &&
                fetcher.data.wheelId === wheel.id &&
                Boolean(fetcher.data.success))) ? (
              <button
            ...
            <div
              className="studio-console-message-region"
              aria-live="polite"
              aria-atomic="true"
            >
              {actionMessage?.error ? (
            ...
            <details className="studio-entry-preview">
              <summary>View current wheel order</summary>
