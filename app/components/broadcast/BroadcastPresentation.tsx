  ...
  return (
    <section className={`broadcast-presentation${info.spinning ? " is-spinning" : ""}`}>
      <header className="broadcast-presentation-header">
        ...
        <div className="broadcast-presentation-title"><p>ASYLUM GAMES LIVE</p><h1>{info.gameTitle}</h1></div>
        <div className="broadcast-presentation-id"><span>RAFFLE</span><strong>{info.raffleCode}</strong><small>{info.wheelLabel ?? "Awaiting production"}{info.wheelSequence ? ` · WHEEL ${info.wheelSequence}` : ""}</small></div>
      </header>
      ...
      <main className="broadcast-presentation-stage">
        ...
        <div className="broadcast-state-bug">{info.wheelStatus.replace("_", " ")}</div>
      </main>
      <section className="broadcast-presentation-lower">
        <article className="broadcast-winner-area">
          <span>{healthState === "REWARD_CHAMBER" ? "REWARD CHAMBER" : "CURRENT WINNER"}</span>
          <strong>{healthState === "REWARD_CHAMBER" ? info.reward ?? "READY" : info.winner ?? (info.spinning ? "CONTAINMENT IN PROGRESS" : "READY")}</strong>
        ...
        <article className="broadcast-second-area">
          ...
          <strong>{info.secondChance.length ? info.secondChance.map((item) => item.value).join(" · ") : "PENDING"}</strong>
        </article>
      </section>
      {overlay}
      <footer className="broadcast-presentation-footer"><span>ASYLUMGAMES.COM</span><span>FACEBOOK · ASYLUM GAMES</span><span>UPCOMING PRIZE · {info.upNext ?? info.bonus ?? "TO BE ANNOUNCED"}</span></footer>
    </section>
  );
}
