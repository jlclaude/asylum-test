  ...
  return (
    <section className="prize-game-panel" aria-labelledby="prize-game-heading">
      <header>
        ...
        <h2 id="prize-game-heading">PRIZE CLAIM</h2>
      </header>
      {fetcher.data?.error ? (
      ...
      {eligibleWheels.map((wheel) => {
        ...
        const immediateUrl =
          fetcher.data?.wheelId === wheel.id ? fetcher.data.privateUrl : null;
        return (
          <article className="prize-game-card" key={wheel.id}>
            ...
            {immediateUrl ? (
              <div className="prize-private-link">
                ...
                <button type="button" onClick={() => void copyImmediateLink()}>
            ...
            {saved && !immediateUrl ? (
              <p className="prize-link-note">
                Open Admin Detail to copy an available claim link again. Token
                ending: {saved.tokenLastFour}
              </p>
            ) : null}
            <div className="prize-actions">
              ...
              {saved?.status === "OPEN" ? (
                <fetcher.Form method="post">
                  ...
                  />
                  <input type="hidden" name="prizeClaimId" value={saved.id} />
                  <input type="hidden" name="wheelId" value={wheel.id} />
                  {csrfToken ? (
                    <input type="hidden" name="csrfToken" value={csrfToken} />
                  ) : null}
              ...
              {saved && ["SUBMITTED", "REVIEWED"].includes(saved.status) ? (
                <fetcher.Form method="post">
                  ...
                  <button disabled={busy}>Mark Fulfilled</button>
                </fetcher.Form>
              ) : null}
            </div>
          </article>
