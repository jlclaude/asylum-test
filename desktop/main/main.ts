...
function registerIpc() {
  ...
  ipcMain.handle("broadcast:set-scale", (event, scale: unknown) => { if (!fromShell(event) || !broadcastView || !["fit", 1, 1.25, 1.5].includes(scale as never)) return false; broadcastView.webContents.setZoomFactor(1); void broadcastView.webContents.executeJavaScript(`window.dispatchEvent(new CustomEvent("asylum-broadcast-scale", { detail: ${JSON.stringify(scale)} }))`); return true; });
