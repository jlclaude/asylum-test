...
declare global {
  ...
  interface ObsState { connection: ObsConnectionState; scenes: string[]; currentScene: string | null; streaming: boolean; recording: boolean; streamDurationMs: number | null; lastError: string | null; }
  ...
  interface Window { asylumDesktop: {
    ...
    obs: {
      settings(): Promise<ObsResult<{ config: { host: string; port: number }; settings: { rememberSettings: boolean; passwordStored: boolean } }>>; state(): Promise<ObsResult<ObsState>>;
