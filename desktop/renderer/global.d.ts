...
declare global {
  ...
  interface Window { asylumDesktop: {
    ...
    broadcast: { retry(): Promise<void>; getObsUrl(): Promise<ObsResult<string>>; copyObsUrl(): Promise<ObsResult<string>>; openObsUrl(): Promise<ObsResult<string>>; regenerateObsUrl(): Promise<ObsResult<string>>; getHealth(): Promise<BroadcastHealth | null>; setScale(scale: "fit" | 1 | 1.25 | 1.5): Promise<boolean>; setSafeAreas(visible: boolean): Promise<boolean> };
  ...
  interface ActiveGameContext { gameId: string; sourceUrl: string; broadcastUrl: string; raffleCode: string | null; gameTitle: string | null; hostCsrfToken: string | null; locked: boolean; }
